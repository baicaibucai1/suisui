...
export default function ChangeList() {
  ...
  return (
    <div className="flex max-h-[38%] min-h-0 shrink-0 flex-col border-t border-line">
      <div className="flex h-9 shrink-0 items-center gap-2 pl-4 pr-3">
        <span className="text-[11px] font-semibold tracking-[0.14em] text-ink-3">待同步</span>
        {visible.length > 0 && (
          <span className="rounded-full bg-surface px-1.5 py-px text-[10.5px] font-medium tabular-nums text-ink-2">
            {visible.length}
          </span>
        )}
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-2.5 pb-3">
        {changes.length === 0 ? (
          <div className="flex items-start gap-2 px-1.5 pb-2 text-[12px] leading-relaxed text-ink-3">
            <Check size={13} strokeWidth={2} className="mt-[2px] shrink-0 text-ok" />
            <span>本地和远端一致</span>
          </div>
        ) : (
          <>
              ...
              <div className="space-y-[1px]">
                {visible.map((c) => (
                  <button
                    ...
                    className="flex w-full items-center gap-2 overflow-hidden rounded-[6px] px-1.5 py-[5px] text-left transition-colors hover:bg-surface-2"
                  >
                    <span className={`shrink-0 ${kindColor(c.kind)}`}>{kindIcon(c.kind)}</span>
                    <span className="min-w-0 flex-1 truncate whitespace-nowrap font-mono text-[11px] text-ink-2">
                      {c.path}
                    </span>
                    <span className={`shrink-0 text-[10.5px] ${kindColor(c.kind)}`}>{kindLabel(c.kind)}</span>
                  </button>
                ))}
              </div>
            )}
            ...
            {hidden > 0 && (
              <button
                ...
                className="mt-1 flex w-full items-center gap-1.5 rounded-[6px] px-1.5 py-[5px] text-left text-[11px] text-ink-3 transition-colors hover:bg-surface-2 hover:text-ink-2"
